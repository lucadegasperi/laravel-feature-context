<?php namespace LucaDegasperi\LaravelFeatureContext;
...
trait AssertionsTrait
{
    ...
    public function assertResponseOk()
    {
        ...
        return assertTrue($response->isOk(), 'Expected status code 200, got ' .$actual);
    }
    ...
    {
        return assertEquals($code, $this->client->getResponse()->getStatusCode());
    }
    ...
    public function assertRedirectedTo($uri, $with = array())
    {
        ...
        assertInstanceOf('Illuminate\Http\RedirectResponse', $response);

        assertEquals($this->app['url']->to($uri), $response->headers->get('Location'));
    ...
    public function assertSessionHas($key, $value = null)
    {
        ...
        if (is_null($value))
        {
            assertTrue($this->app['session.store']->has($key), "Session missing key: $key");
        }
        else
        {
            assertEquals($value, $this->app['session.store']->get($key));
        }
    }
    ...
    public function assertSessionHasErrors($bindings = array(), $format = null)
    {
        ...
        foreach ($bindings as $key => $value)
        {
            if (is_int($key))
            {
                assertTrue($errors->has($value), "Session missing error: $value");
            }
            else
            {
                assertContains($value, $errors->get($key, $format));
            }
        }
    }
